...
def textMinePDF(filepath, dpi=200, thread_count=4, bottom_margin=14/15, n_ngram=3,
    ...
    for p in english_paragraphs:
        ...
        cleaned_paragraphs.append(' '.join(r))
    ...
    return (pd.DataFrame(list(zip([resolution for i in range(len(cleaned_paragraphs))], 
                           [series for i in range(len(cleaned_paragraphs))],
                           cleaned_paragraphs)), columns =['resolution', 'series','item']),
